...
#'   which is the weight for scorestatistics. The default value is "c(1,25)".
#'
#' @return
...
#'
#' @export
mist <- function(y, X, G, Z, method = "liu", model = c("guess", "continuous", "binary"), weight = FALSE, weight.beta = c(1, 25)) {
  ...
  switch(
    ...
    "continuous" = {
      ...
      if (weight) {
        ...
        suppressMessages(mist_linear(y = y, X = X, G = G, Z = Z, method = method, weight.beta = weight.beta))
      }
    },
    "binary" = {
      ...
      if (weight) {
        ...
        suppressMessages(mist_logit(y = y, X = X, G = G, Z = Z, method = method, weight.beta = weight.beta))
      }
    },
...
#' @return data.frame
#' @export
mist_logit_weight <- function(y, X, G, Z, method = "liu", weight.beta = c(1, 25)) {
  if (is.vector(y, "numeric")) stop('"y" must be a numeric vector.')
  if (is.matrix(G) & is.numeric(G)) stop('"G", must be a numeric matrix.')
  if (is.matrix(X) & is.numeric(X)) stop('"X", must be a numeric matrix.')
  if (is.matrix(Z) & is.numeric(Z)) stop('"Z", must be a numeric matrix.')
...
#' @return data.frame
#' @export
mist_linear_weight <- function(y, X, G, Z, method = "liu", weight.beta = c(1, 25)) {
  if (is.vector(y, "numeric")) stop('"y" must be a numeric vector.')
  if (is.matrix(G) & is.numeric(G)) stop('"G", must be a numeric matrix.')
  if (is.matrix(X) & is.numeric(X)) stop('"X", must be a numeric matrix.')
  if (is.matrix(Z) & is.numeric(Z)) stop('"Z", must be a numeric matrix.')
