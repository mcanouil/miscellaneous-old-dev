#' read_idats
#'
#' @param directory [character] Location of IDAT files, 
#'     default is the current working directory.
#' @param csv_file [character] Path to the sample sheet (csv files) or 
#'     name of the sample sheet in \code{directory}.
#' @param meth_value_type [character] Indicates whether you prefer m-values (\code{"M"}) 
#'     or beta-values (\code{"B"}). Default is \code{"B"}.
#' @param filter_beads [logical] Should probes with a beadcount less than 3 will be removed 
#'     depending on the \code{bead_cutoff} value. Default is \code{TRUE}.
#' @param bead_cutoff [numeric] The beadCutoff represents the fraction of samples that must 
#'     have a beadcount less than 3 before the probe is removed.(default = 0.05)
#' @param filter_non_cpg [logical] Should non-cg probes be removed? Default is \code{TRUE}.
#' @param filter_snps [logical] Should probes in which the probed CpG falls near a 
#'     SNP be removed? Default is \code{TRUE}.
#' @param population [vector(character)]
#' @param filter_multihit [logical] Should probes in which the probe aligns to multiple 
#'     locations with bwa be removed? Default is \code{TRUE}.
#' @param filter_xy [logical] Should probes from X and Y chromosomes be removed? Default is \code{TRUE}.
#' @param array_name [character] Choose microarray type, eiyther \code{"450K"} or \code{"EPIC"}.
...
read_idats <- function(
  ...
  stopifnot(suppressPackageStartupMessages(
    require("minfi") & 
      switch(
        EXPR = array_name,
        "450k" = require("IlluminaHumanMethylation450kmanifest"),
  ...
  if (filter_snps) {
    manifest_hg19 <- switch(
      EXPR = array_name,
      "450k" = {
        get(data("hm450.manifest.hg19", package = "ChAMPdata"))
      },
      "EPIC" = {
        get(data("EPIC.manifest.hg19", package = "ChAMPdata"))
      }
    )
  ...
  if (filter_multihit) {
    multi_hit <- get(data("multi.hit", package = "ChAMPdata"))
  ...
  if (filter_xy) {
    switch(
      EXPR = array_name,
      "450k" = data("probe.features", package = "ChAMPdata"),
      "EPIC" = data("probe.features.epic", package = "ChAMPdata")
    )
...
#' @param ignore.case [logical] A logical value. 
#'     If \code{TRUE}, the directory path is prepended to the file names to give a relative file path. 
#'     If \code{FALSE}, the file names (rather than paths) are returned.
#' @param recursive [logical] Should the listing recurse into directories?
#' @param full.names [logical] Should pattern-matching be case-insensitive?
#'
#' @keywords internal
read_sample_sheet <- function(
  ...
  sample_sheet <- read.csv(list_files, stringsAsFactor = FALSE, skip = data_header)
...
#' read_metharray
#'
#' @param basenames The \code{basenames} or \code{filenames} of the IDAT files. 
#'     \code{basenames} are the filename without the ending \code{_Grn.idat} or \code{_Red.idat}.
#'     \code{filenames} are filenames including \code{_Grn.idat} or \code{_Red.idat}.
#' @inheritParams read_idats 
#'
#' @keywords internal
