...
createNanoStringSet <- function(endogenous, positiveControl, negativeControl, housekeepingControl, designs) {
  ...
  res <- new(
    "NanoStringSet",
    exprs = endogenous, phenoData = designs,
...
estNormalizationFactors <- function(NanoStringData, kit = c("PlexSet", "miRNA")) {
  ...
  for (i in 1:n) {
    ...
    lambda_i[i] <- round(model$coeff[1])
    c[i] <- model$coeff[2]
  }
  c <- c / mean(c)
  names(lambda_i) <- colnames(pos)
  names(c) <- colnames(pos)
  lambda_i[lambda_i == 0] <- 1

  positiveFactor(NanoStringData) <- c
  negativeFactor(NanoStringData) <- lambda_i
  ...
  if (nrow(housekeepingControl(NanoStringData)) == 0) {
    ...
    house1 <- sweep(housekeepingControl(NanoStringData), 2, lambda_i, FUN = "-")
    house1[house1 <= 0] <- 1
...
normaliseExprsNanoString <- function(NanoStringData, x = Biobase::exprs(NanoStringData)) {
  c <- positiveFactor(NanoStringData)[colnames(x)]
  d <- housekeepingFactor(NanoStringData)[colnames(x)]
  k <- c * d
  lambda_i <- negativeFactor(NanoStringData)[colnames(x)]
  Y_n <- sweep(x, 2, lambda_i, FUN = "-")
...
read_mirna <- function(samplesheetpath, design, verbose = TRUE) {
  formatLog <- function(output) {
    NanoStringData.log <- capture.output({
      output
    }) %>%
      gsub("^ ", "", .) %>%
      gsub(" $", "", .) %>%
  ...
  RCC.data <- Reduce(
    f = function(x, y) {
      merge(x = x, y = y, by = c("CodeClass", "Name", "Accession"), all = TRUE)
    },
  ...
  NS_log <- formatLog({
    NanoStringData <- createNanoStringSetFromCsv(
  ...
  if (nrow(housekeepingControl(NanoStringData)) != 0) {
    ...
    p <- cowplot::plot_grid(
      ggplot(data = ggdata_neg, aes(x = ID, y = Value^yScalePower, colour = G)) +
        ...
        labs(x = NULL, y = bquote("Count"^.(yScalePower))),
...
checkOutliers <- function(
                          ...
                          outliers.thresh = 3,
                          n.comp = 5,
                          max.comp = ncol(Biobase::exprs(NanoStringData)) - 1 - n.comp) {
  if (length(positiveFactor(NanoStringData)) == 0 | length(negativeFactor(NanoStringData)) == 0) {
    normalised <- FALSE
  }
  for (iwhat in what) {
    if (normalised) {
      if (iwhat == "controls") {
        ...
        data <- normaliseExprsNanoString(NanoStringData = NanoStringData, x = Biobase::exprs(NanoStringData))
      }
    } else {
      if (iwhat == "controls") {
        if (nrow(housekeepingControl(NanoStringData)) != 0) {
        ...
        data <- Biobase::exprs(NanoStringData)
      }
    }
  ...
  new_exprs <- new.env()
  assign(x = "exprs", value = Biobase::exprs(NanoStringData)[, samples_to_keep], envir = new_exprs)
