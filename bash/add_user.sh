...
[ -d /media/User/$USER ] || mkdir -p /media/User/$USER \
&& useradd -u $ID $USER --home /media/User/$USER \
&& echo "$USER:$PWD" | chpasswd \
&& chown -R $USER:info /media/User/$USER \
&& addgroup $USER info \
&& addgroup $USER root \
&& adduser $USER sudo \
...
useDevtools="1" 
useInternet2="1"' > /media/User/$USER/.rstudio/monitored/user-settings/user-settings \
